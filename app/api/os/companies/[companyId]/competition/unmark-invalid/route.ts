// app/api/os/companies/[companyId]/competition/unmark-invalid/route.ts
// Remove a domain from invalidCompetitors.
...
export async function POST(
  ...
  try {
    ...
    const next = current.filter(d => d.toLowerCase() !== normalized);

    const provenance = createProvenance('competitor_lab', { confidence: 0.9, notes: 'User unmarked invalid competitor' });
    ...
    console.error('[competition/unmark-invalid] Error:', error);
