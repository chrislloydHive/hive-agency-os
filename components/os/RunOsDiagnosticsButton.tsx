...
interface RunOsDiagnosticsButtonProps {
  companyId: string; // Airtable record ID
  companyName: string;
  websiteUrl?: string;
}

interface OsRunStatus {
  runId: string;
...
export default function RunOsDiagnosticsButton({
  companyId,
  companyName,
  websiteUrl,
}: RunOsDiagnosticsButtonProps) {
  const router = useRouter();
  const [isRunning, setIsRunning] = useState(false);
  const [runStatus, setRunStatus] = useState<OsRunStatus | null>(null);
  ...
  useEffect(() => {
    ...
    if (runStatus.status === 'completed' || runStatus.status === 'failed') {
      ...
      // Refresh the page data when completed to show new diagnostics
    ...
    const pollInterval = setInterval(async () => {
      try {
        ...
        if (!response.ok) {
          console.error('Failed to fetch OS run status:', response.statusText);
          return;
        }
        ...
        if (data.ok) {
          ...
          if (data.status === 'completed' || data.status === 'failed') {
            ...
            // Refresh diagnostics data on completion
        ...
        console.error('Error polling OS run status:', error);
      }
    }, 4000);
  ...
  const handleRunOsDiagnostics = useCallback(async () => {
    if (!websiteUrl) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.error || 'Failed to start OS diagnostics run');
      }
      ...
      console.error('Error starting OS diagnostics run:', error);
      setErrorMessage(
        error instanceof Error ? error.message : 'Failed to start OS diagnostics run'
      );
      setIsRunning(false);
    }
        ...
        <div className="flex items-start gap-2">
          ...
          <div className="flex-1">
            <p className="text-sm font-medium text-green-400">
              OS diagnostics completed successfully
            </p>
            <p className="text-xs text-slate-400 mt-0.5">
              Diagnostics data has been refreshed
            </p>
          </div>
        </div>
      );
    ...
    if (status === 'failed') {
      return (
        <div className="flex items-start gap-2">
          ...
          <div className="flex-1">
            <p className="text-sm font-medium text-red-400">OS diagnostics failed</p>
            {error && (
  ...
  return (
    <div className="rounded-xl border border-slate-800 bg-slate-900 p-4 flex flex-col gap-3">
      ...
      {runStatus ? (
        ...
        <button
          onClick={handleRunOsDiagnostics}
