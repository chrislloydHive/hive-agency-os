...
1. **`POST /api/review/assets/approve`**: sets **Asset Approved (Client)** (and related fields) on CRAS via **`setSingleAssetApprovedClient`**, then calls **`triggerPartnerDeliveryForCras`** (same logic as **`POST /api/delivery/partner/approved`**) to emit Inngest **`partner.delivery.requested`** so **`partner-delivery-requested`** can copy to the partner package and (when configured) add **`_Production Assets`**. Bulk approve does the same per row.
