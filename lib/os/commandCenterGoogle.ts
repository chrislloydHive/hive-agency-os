...
export const COLD_OUTREACH_RE =
  /\b(cold\s+email|outbound\s+(system|leads?|campaign)|done[- ]for[- ]you\s+(outbound|lead|system)|qualified\s+leads|pre[- ]?vetted\s+(leads?|prospects?)|book\s+(a\s+)?call|get\s+booked|15[- ]?min(ute)?\s+(call|chat|of\s+your\s+time)|\d+\s*minutes?\s+of\s+your\s+time|schedule\s+a\s+(demo|call|meeting)|quick\s+question|saw\s+your\s+(company|profile|website|linkedin|recent)|reaching\s+out\s+because|reaching\s+back\s+out|thought\s+you.d\s+be\s+interested|would\s+love\s+to\s+(re)?connect|love\s+to\s+(re)?connect|open\s+to\s+a\s+quick\s+chat|let.s\s+hop\s+on\s+a\s+call|we\s+help\s+(companies|businesses|agencies|founders|brands)|(increase|grow|3x|2x|10x|scale)\s+(your\s+)?(pipeline|revenue|leads?|sales|conversions?|roas|advertising|marketing|media)|(scale|grow|expand|build)\s+(the|your)\s+(advertising|marketing|media|paid|digital)\s+(side|business|practice|operation|arm)|generate\s+(more\s+)?leads|partnership\s+opportunit|strategic\s+partnership|circling\s+back|circle\s+back|just\s+following\s+up|following\s+up\s+on\s+my\s+(previous|last)|(my\s+)?previous\s+(email|message|note)|just\s+(checking\s+in|wanted\s+to\s+(check|follow))|did\s+(you|this)\s+(get\s+a\s+chance|land|come\s+through|fall\s+through)|bumping\s+this\s+(up|to\s+the\s+top)|\bICP\b|ideal\s+customer|introduce\s+(myself|us|our\s+(company|firm|agency))|on\s+behalf\s+of\s+\w+|investing\s+in|help\s+you\s+(scale|grow|reach)|exclusive\s+(opportunit|invite|access)|free\s+(trial|consultation|audit|strategy\s+session)|complimentary\s+(audit|consultation|session)|hope\s+(all|this|everything)\s+is\s+well|hope\s+you.?re\s+(doing\s+)?well|congrats\s+on\s+(your|the|the\s+recent)\s+(launch|new|recent|raise|round|growth|news|win|hire)|congratulations\s+on\s+(your|the)|as\s+you\s+(look\s+to|continue\s+to|think\s+about)\s+(scale|scaling|grow|growing|expand|expanding|build|building)|learn\s+more\s+about\s+what\s+you\s+(and\s+(your|the)\s+\w+\s+(team|crew|group)\s+)?are\s+doing|opportunities\s+on\s+(your|the)\s+radar|share\s+how\s+we.?re\s+(helping|working\s+with)|improve\s+(their|your)\s+(media|advertising|marketing|paid|digital)\s+(performance|efficiency|results|spend|outcomes?)|(in|across|throughout)\s+the\s+(digital|programmatic|paid\s+media|advertising|marketing)\s+(space|world|landscape|industry)|wanted\s+to\s+(introduce|reach\s+out|drop\s+a\s+line)|been\s+a\s+while|been\s+(some\s+)?time)\b/i;
...
export async function fetchTriageInbox(
  ...
  try {
    ...
    // -from:me on every query strips out self-sent mail at the Gmail level —
    // crucial because Hive OS itself emails Chris a daily task digest. Without
    // this, the digest becomes a triage candidate, which becomes a task,
    // which feeds the next digest. Self-recursive noise.
    const queries: Array<{ q: string; reason: string }> = [
      { q: `in:inbox category:primary is:unread newer_than:${days}d -from:me`, reason: 'Unread primary' },
      { q: `in:inbox is:starred -from:me`, reason: 'Starred' },
      { q: `in:inbox is:important newer_than:${days}d -from:me`, reason: 'Important' },
      ...(importantDomains.length
        ? [{ q: `in:inbox (${domainClause}) newer_than:${days}d -from:me`, reason: 'Key sender' }]
        : []),
    ];
