// lib/os/refreshTaskFromThreadHandler.ts
// Shared POST handler for task thread refresh (used by nested and flat API routes).
...
import { THREAD_GONE_CODE } from '@/lib/os/gmailThreadGone';

export async function refreshTaskFromThreadHandler(taskId: string): Promise<NextResponse> {
  try {
    ...
    if (!result.ok) {
      if (result.code === THREAD_GONE_CODE || result.status === 410) {
        return NextResponse.json(
          { code: THREAD_GONE_CODE, error: result.error },
          { status: 410 },
        );
      }
