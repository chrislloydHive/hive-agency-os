...
export async function runInitStep(state: GapRunState, _timeLeftMs: number): Promise<void> {
  ...
  log('━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━');
  log('📈 GENERATING GROWTH ACCELERATION PLAN (GAP)');
  log(`📊 URL: ${state.url}`);
  if (state.competitors.length > 0) {
    log(`🔍 Competitors: ${state.competitors.length}`);
...
export async function runAssessmentStep(state: GapRunState, timeLeftMs: number): Promise<void> {
  ...
  try {
    ...
    const timeoutPromise = new Promise<null>((resolve) => 
      setTimeout(() => {
        timedOut = true;
        log(`⚠️  Assessment generation timed out after ${Math.round(assessmentTimeout / 1000)}s, using fallback`);
        resolve(null);
      }, assessmentTimeout)
    );
    ...
    if (timedOut) {
      log(`⏱️  Assessment timed out after ${Math.round(assessmentElapsed / 1000)}s (budget: ${Math.round(assessmentTimeout / 1000)}s)`);
      assessment = null; // Ensure we use fallback
    } else if (assessment) {
      log(`✅ Assessment completed successfully in ${Math.round(assessmentElapsed / 1000)}s`);
    }
  } catch (error: any) {
    const assessmentElapsed = Date.now() - assessmentStartTime;
    log(`⚠️  Assessment generation failed after ${Math.round(assessmentElapsed / 1000)}s: ${error?.message || String(error)}`);
  ...
  if (!assessment) {
    log('📋 Building fallback assessment from minimal data...');
...
export async function runFetchHtmlStep(state: GapRunState, _timeLeftMs: number): Promise<void> {
  ...
  console.log('📄 Step 1.5: Fetching HTML and extracting page elements...');
  ...
  try {
    ...
    log(`⚠️  Failed to fetch HTML: ${htmlError?.message || 'Unknown error'}`);
    // Continue with empty HTML - we'll build a minimal plan
  }
  
  if (!mainPageHTML) {
    log('⚠️  No HTML available - using minimal fallback data');
    mainPageHTML = ''; // Use empty string as fallback
  }
  htmlByUrl[state.url] = mainPageHTML;
  successfulUrls.push(state.url);
  console.log(`✅ Fetched HTML for main page (${mainPageHTML.length} chars)`);
...
export async function runExtractFeaturesStep(state: GapRunState, _timeLeftMs: number): Promise<void> {
  ...
  console.log('📊 Step 0: Extracting site features from HTML...');
  ...
  console.log(`✅ SiteFeatures extracted (${Object.keys(features).length} signal categories)`);
...
export function runDiscoverPagesStep(state: GapRunState): void {
  ...
  console.log('🔍 Discovering key pages from navigation and footer...');
  ...
  console.log(`✅ Discovered ${discoveredPages.length} key page(s) to fetch`);
...
export async function runFetchExtraPagesStep(state: GapRunState, timeLeftMs: number): Promise<void> {
  ...
  if (pagesToFetch.length > 0) {
    console.log(`📥 Fetching ${pagesToFetch.length} page(s) in parallel...`);
    const fetchPromises = pagesToFetch.map(async (pageUrl) => {
      ...
      try {
        ...
        log(`⚠️  Failed to fetch ${pageUrl}: ${err?.message || 'Unknown error'}`);
        failedUrls.push(pageUrl);
        return { url: pageUrl, success: false };
      }
    });
  ...
  console.log(`✅ Site element context extracted (${siteElementContext.pages.length} pages)`);
  ...
  if (state.competitors.length > 0) {
    console.log(`🔍 Step 1.6: Fetching HTML and extracting page elements for competitors...`);
    const competitorPromises = state.competitors.slice(0, 3).map(async (compUrl) => {
      try {
        ...
        log(`⚠️  Failed to fetch competitor ${compUrl}: ${err?.message || 'Unknown error'}`);
      }
      return null;
    });
...
export async function runContentInventoryStep(state: GapRunState, _timeLeftMs: number): Promise<void> {
  ...
  console.log('📚 Step 2.4: Analyzing content inventory...');
...
export async function runTechnicalSeoStep(state: GapRunState, _timeLeftMs: number): Promise<void> {
  ...
  if (state.technicalSeoSignals) {
    log('✅ Technical SEO signals already computed, skipping');
    return;
  }
  
  log('🔍 Step 2.4b: Extracting technical SEO signals...');
  ...
  try {
    ...
    log(`✅ Technical SEO signals extracted:`);
...
export async function generateGrowthAccelerationPlan(
  ...
  if (options?.gapIaData) {
    ...
    log('✅ Reusing GAP-IA analysis data (skipping redundant assessment)...');
    ...
    if (maturityOverride) {
      log(`🏆 Applying maturity override: ${maturityOverride.reason}`);
      ...
      log(`  Brand: ${iaData.dimensions?.brand?.score || iaData.core?.brand?.brandScore || 50} → ${brandScore}`);
      log(`  Content: ${iaData.dimensions?.content?.score || iaData.core?.content?.contentScore || 50} → ${contentScore}`);
      log(`  SEO: ${iaData.dimensions?.seo?.score || iaData.core?.seo?.seoScore || 50} → ${seoScore}`);
      log(`  Website: ${iaData.dimensions?.website?.score || iaData.core?.website?.websiteScore || 50} → ${websiteScore}`);
      log(`  Overall: ${iaData.summary?.overallScore || iaData.core?.overallScore || 50} → ${overallScore}`);
    }
    ...
    log('✅ Assessment built from GAP-IA data');
  } else {
    ...
    try {
      ...
      log(`⚠️  Assessment generation failed or timed out: ${error?.message || String(error)}`);
      assessmentError = error;
      // Don't throw - we'll build a fallback GAP plan instead
    }
  }
  ...
  if (pagesToFetch.length > 0) {
    ...
    const fetchPromises = pagesToFetch.map(async (pageUrl) => {
      ...
      try {
        ...
        if (pageHTML) {
          htmlByUrl[pageUrl] = pageHTML;
          successfulUrls.push(pageUrl);
          console.log(`✅ Fetched HTML for ${pageUrl} (${pageHTML.length} chars)`);
          return { url: pageUrl, success: true };
        } else {
          failedUrls.push(pageUrl);
          console.warn(`⚠️  Failed to fetch HTML for ${pageUrl} (timeout or empty)`);
          return { url: pageUrl, success: false };
        }
      } catch (error) {
        failedUrls.push(pageUrl);
        console.warn(`⚠️  Error fetching ${pageUrl}:`, error);
        return { url: pageUrl, success: false };
      }
    });
    ...
    await Promise.allSettled(fetchPromises);
    console.log(`✅ Completed fetching ${successfulUrls.length} page(s) successfully, ${failedUrls.length} failed`);
  }
  ...
  if (siteElementContext.caseStudies.length > 0) {
    console.log(`   📊 Found ${siteElementContext.caseStudies.length} case study(ies)`);
    await reportProgress(`Found ${siteElementContext.caseStudies.length} case stud${siteElementContext.caseStudies.length > 1 ? 'ies' : 'y'}`, 20, 'extracting-context');
  }
  ...
  const competitorContexts: SiteElementContext[] = [];
  if (competitors.length > 0) {
    console.log(`🔍 Step 1.6: Fetching HTML and extracting page elements for ${competitors.length} competitor(s) in parallel...`);
    ...
    const competitorPromises = competitors.map(async (competitorUrl) => {
      try {
        ...
        if (competitorHTML) {
          ...
          console.log(`✅ Fetched HTML for competitor ${competitorUrl} (${competitorHTML.length} chars)`);
          ...
          console.log(`✅ Extracted structured context from competitor ${competitorUrl} (${competitorContext.pages.length} page(s))`);
          return competitorContext;
        } else {
          console.warn(`⚠️  Failed to fetch HTML for competitor: ${competitorUrl} (timeout or empty)`);
          return null;
        }
      } catch (error) {
        console.error(`❌ Error processing competitor ${competitorUrl}:`, error);
        return null;
      }
    });
    ...
    console.log(`✅ Processed ${competitorContexts.length} competitor context(s)`);
  }
  ...
  const contentInventory = await Promise.race([
    ...
    new Promise<Awaited<ReturnType<typeof analyzeContentInventory>>>((resolve) => 
      setTimeout(() => {
        console.warn('⚠️  Content inventory analysis timeout, using fallback');
        resolve({
          blogPostsFound: 0,
          blogCategories: [],
          caseStudiesFound: 0,
  ...
  console.log('🔍 Step 2.4b: Extracting technical SEO signals...');
  const technicalSeoSignals = await extractTechnicalSeoSignals(url, assessment, siteElementContext);
  console.log(`✅ Technical SEO signals extracted:`);
  ...
  console.log(`📊 Detected maturity level: ${detectedMaturity}`);
  await reportProgress(`Detected maturity: ${detectedMaturity.replace('-', ' ')}`, 30, 'maturity-detection');

  console.log('🔍 Step 2: Generating section analyses...');
  ...
  console.log('✅ Section analyses complete (except Brand & Positioning)');
  ...
  console.log('🔍 Step 2.5: Analyzing competitors, market, and positioning in parallel (with timeout protection)...');
  ...
  if (marketAnalysisResult.status === 'fulfilled') {
    ...
    console.warn('⚠️  Market analysis failed:', marketAnalysisResult.reason);
  ...
  if (competitorAnalysisResult.status === 'fulfilled' && competitorAnalysisResult.value) {
    ...
    console.log(`✅ Competitor analysis complete (${competitorAnalysis.competitorsReviewed.length} competitor(s) reviewed)`);
    ...
    console.warn('⚠️  Competitor analysis failed:', competitorAnalysisResult.reason);
  }
  ...
  if (positioningAnalysisResult.status === 'fulfilled') {
    ...
    console.warn('⚠️  Positioning analysis failed:', positioningAnalysisResult.reason);
  ...
  // Wrap in timeout to prevent blocking
  console.log('🔍 Step 2.6: Analyzing Brand & Positioning (strategic focus)...');
  ...
  console.log('✅ Brand & Positioning analysis complete');
  ...
  console.log('⚡ Step 3: Deriving quick wins from section analyses...');
  ...
  console.log(`✅ Derived ${quickWins.length} quick wins from section analyses`);
  ...
  console.log('🎯 Step 4: Deriving strategic initiatives from section analyses...');
  ...
  console.log(`✅ Derived ${strategicInitiatives.length} strategic initiatives from section analyses`);
  ...
  console.log('📊 Building dimension scores from component data...');
  ...
  console.info(`📊 All Dimension Scores:`, {
    website: websiteScore,
    content: contentScore,
    seo: seoScore,
    brand: brandScore,
  ...
  if ('error' in plan) {
    console.error('❌ Growth Acceleration Plan (GAP) validation failed - returning fallback error object');
  ...
  console.log('✅ Growth Acceleration Plan (GAP) generated successfully');
...
export async function generateQuickWins(
  ...
  console.log('🤖 Generating quick wins with AI...');
...
Your job:
- Propose 3–5 improvements that can be implemented within 1–2 weeks.
...
- "immediate" — can be started right away (0–2 weeks)
- "short_term" — next priorities (2–8 weeks)
...
Generate 3-5 concrete, site-specific quick wins that reference actual elements from this website.`;
  ...
  try {
    ...
    console.log(`✅ Generated ${validatedQuickWins.length} quick wins`);
    ...
    if (error?.message?.includes('timed out') || error?.message?.includes('Request timed out')) {
      console.warn('⚠️  Quick wins generation timed out, using fallback');
      return [];
    }
    console.error('❌ Error generating quick wins with AI:', error);
    // Fallback to basic quick wins if AI fails
    console.log('⚠️  Falling back to basic quick wins from assessment');
...
export async function generateStrategicInitiatives(
  ...
  console.log('🤖 Generating strategic initiatives with AI...');
  ...
  const systemPrompt = `You are a senior marketing strategist planning 3–5 STRATEGIC INITIATIVES for the next 60-180 days, written for a founder or small marketing team.

INTRO SENTENCE REQUIREMENT:
At the top of your response JSON, include an "introSentence" field with:
"These are longer-term projects (60–180 days) that will fundamentally improve your marketing system."
...
Your job:
- Define 3–5 larger initiatives that can be executed over 60-180 days.
...
- "medium_term" — foundation or bigger changes (2–6 months)
- "long_term" — strategic initiatives (6–18 months)
...
- Make the text competitor-aware and specific (mention actual competitor patterns, content gaps, or ranking opportunities)`;
  ...
  try {
    ...
    console.log(`✅ Generated ${validatedInitiatives.length} strategic initiatives`);
    return validatedInitiatives;
  } catch (error: any) {
    // Check if it's a timeout error - if so, return empty array (outer Promise.race will handle it)
    if (error?.message?.includes('timed out') || error?.message?.includes('Request timed out')) {
      console.warn('⚠️  Strategic initiatives generation timed out, using fallback');
      return [];
    }
    console.error('❌ Error generating strategic initiatives with AI:', error);
    // Fallback to basic strategic initiatives if AI fails
    console.log('⚠️  Falling back to basic strategic initiatives from assessment');
